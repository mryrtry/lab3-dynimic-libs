...
EXPORT struct image transform(const struct image* source) {
    ...
    for (size_t y = 0; y < source->height; y++) {
        for (size_t x = 0; x < source->width; x++) {
            transformed.data[offset_by_coords(&transformed, source->width - x - 1, y)] = source->data[offset_by_coords(source, x, y)];
        }
    }
    return transformed;
}
...
EXPORT struct transformation_plugin init_plugin(void) {
    return (struct transformation_plugin) {.key = "fliph", .transform = transform};
}
